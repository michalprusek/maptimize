...
async def test_query_endpoint_resolves_group_and_returns_rows(mock_db):
    ...
    with patch("routers.query.get_user_group_ids", AsyncMock(return_value=[3])) as group, \
    ...
    assert kwargs["user_id"] == 7 and kwargs["group_ids"] == [3] and kwargs["limit"] == 50


async def test_query_endpoint_maps_service_error_to_400(mock_db):
    user = SimpleNamespace(id=1)
    with patch("routers.query.get_user_group_ids", AsyncMock(return_value=[])), \
